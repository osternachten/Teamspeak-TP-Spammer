...
const char* ts3plugin_name() {
	...
	if (!result) {
		const wchar_t* name = L"Anti GommeHD - All u can do";
...
void ts3plugin_initMenus(struct PluginMenuItem*** menuItems, char** menuIcon) {
	...
	CREATE_MENU_ITEM(PLUGIN_MENU_TYPE_CHANNEL, TPSPAM, "TP Spammen", "1.png");
...
bool tp_spam = true;
void ThreadLoop_TP_Spam() {
	int sleep = 750;
	if (ts3Functions.requestIsTalker(serverConn, 0, "", NULL) != ERROR_ok) printf("ERROR");
	std::this_thread::sleep_for(std::chrono::milliseconds(sleep));
	int count = 0;
	while (tp_spam)
	{
		count += 1;
		if (ts3Functions.requestIsTalker(serverConn, 1, "", NULL) != ERROR_ok) printf("ERROR");
		std::this_thread::sleep_for(std::chrono::milliseconds(sleep));

		if (ts3Functions.requestIsTalker(serverConn, 0, "", NULL) != ERROR_ok) printf("ERROR");
		std::this_thread::sleep_for(std::chrono::milliseconds(sleep));

		if (count >= 5) tp_spam = false;
	}
	if (ts3Functions.requestIsTalker(serverConn, 1, "Gib ma Talkpower", NULL) != ERROR_ok) printf("ERROR");
	tp_spam = true;
}

void tp_spam_function() {
	std::thread Q(ThreadLoop_TP_Spam);
	Q.detach();
}
/*
